...
object MapImplicits {
  implicit class SqlEntriesPerTransactionEnhancement(val entries: SqlEntriesPerTransaction) extends AnyVal {
    def toSql: SqlsPerTransaction = {
      entries.map { case (transaction, entry) =>
        val head =
          s"""
             |----------------------------------------------------------------------------------------
             |-- Transaction group: ${transaction.toString.toUpperCase}
             |----------------------------------------------------------------------------------------
             |""".stripMargin
        transaction -> (head +: entry.map(_.sqlExpression))
      }
    }
  }
}
