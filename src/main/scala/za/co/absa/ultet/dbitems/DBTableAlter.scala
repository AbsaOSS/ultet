...
import za.co.absa.ultet.dbitems.DBTable.ColumnsDifferenceResolver
import za.co.absa.ultet.model.{SQLEntry, SchemaName}
import za.co.absa.ultet.model.table.{TableAlteration, TableName}
import za.co.absa.ultet.model.table.alterations.{TablePrimaryKeyAdd, TablePrimaryKeyDrop}
import za.co.absa.ultet.model.table.index.{TableIndexCreate, TableIndexDrop}

case class DBTableAlter(newTable: DBTable, origTable: DBTable) extends DBItem {
  ...
  override def sqlEntries: Seq[SQLEntry] = {
    ...
    val alterationsToAddIndices = addIndices.map(idx => TableIndexCreate(newTable.tableIdentifier, idx))
